...
def get_dict_of_cols(df: pd.DataFrame, n=2, exclude=['id'], method: int = 1, ignore_strings: bool = False):
    """
    method is in {1, 2}
    ...
    if method == 1:
        ...
        for col in df.columns:
            if (
                (ignore_strings or df[col].dtype in ["float64", "int64"]) and \
                df.shape[0] / 100 < len(df[col].unique()) <= df.shape[0]
            ):
                result[col] = []
        return result
...
def remove_outliers(
    ...
    cols = get_dict_of_cols(df=df, n=n, exclude=exclude, method=select_method,ignore_strings=ignore_strings)
